"""
seed_schools.py
Populates the database with sample Jamaican school data.
Uses the SQLAlchemy School model — consistent with the rest of the application.
...
def seed():
    """Insert sample schools using the School SQLAlchemy model."""
    with app.app_context():
        if School.query.count() > 0:
            print("Database already has schools. Skipping to avoid duplicates.")
            print("Delete cdms.db and run again to re-seed.")
            return

        print("Inserting schools...")
        for name, address, contact_person, contact_phone, contact_email, capacity in SCHOOLS_DATA:
        ...
        db.session.commit()
        print(f"Done! Successfully added {len(SCHOOLS_DATA)} schools.")
